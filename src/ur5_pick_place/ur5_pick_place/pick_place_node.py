...
class PickPlaceNode(Node):
    ...
    def move_to(self, positions, label='', retries=3) -> bool:
        ...
        for attempt in range(retries):
            # Création d'un but tout neuf avec le timestamp actuel
            goal = self._build_goal(positions)
            
            # Augmentation progressive de la patience du planificateur au fil des essais
            ...
            # Si code == -4 (Aborted), on attend un peu que les TF se stabilisent
            if code == -4:
                self.get_logger().warn(f'   ⚠️ Temps de trajectoire invalide (code -4), pause...')
                time.sleep(0.5)
